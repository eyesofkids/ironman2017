...
本章的目標是提供上一章的HelloWorld應用的詳盡說明。在這之前，我要先說明幾個學習的重點。

第一，是如果一開始學習React就用官方的例子，例如像TodoMVC或官網現在上面的圈叉遊戲這些例子，如果一下子就看得懂了，那當然代表你的基礎能力非常的好，你也不需要再學什麼。我在一開始學習時，看這些例子我是看不懂的，這個應用裡面有運用太多的新知識需要先學習，如果你沒有這些基礎，你仍然要花很多時間才有辦法把需要的知識補齊，直接看這些例子也是浪費時間而已，一開始先放慢速度先把基礎學會才是我認為比較好的方式。有些人在學這種新的函式庫時，會覺得要像參加旅遊團一樣，走馬看花一天跑十個旅遊景點，把所有的新的、熱門的東西都花點時間學一遍，這樣作的目的是為了什麼？怕聽到專有名詞時不知道是什麼？要記得每一門技術的背後，都是數以百計厲害的工程師共同創作出來的，有這麼簡單讓你幾個小時就學通學會，那這些工程師也不用混了。在一開始學習的時候，如果遇到不懂的地方，哪怕是個很小的地方，也有可能裡面有一些必要的知識需要學得會。

第二，我要再次強調，不要用單一個網頁的方式來寫React程式碼。這也是我認為不正確的學習方式，所謂的直接使用網頁，然後用類似jquery的寫法，把所需的函式庫加到script標籤中，聽起來似乎很便利，但這並不是真實開發的情況，如果真的都是這樣寫就可以，那就不需要大費周章專門為了寫ES6或React的應用，建立一個開發測試的環境。官方提供的範例中，有不少簡單的例子都是這樣子的作法，這些例子都是很簡單到不行的範例。另外，在直接用網頁的方式，實際上還是得經過babel工具編譯，所以還需要加一個babel-core。舉一個之前在討論區中常被問到的問題，React官方的單一個網頁的這些範例一直不改用新版的babel-core函式庫，你知道為什麼嗎？因為新版的babel-core一直無法正確編譯React元件，這裡面有很多bugs。
...
學得精比學得廣來得重要，尤其是在一開始的階段，如果你不能夠的掌握真正的基礎，那麼這段學習時間也是浪費掉而已。再者，知識是一點一滴的學習所累積的結果，每天花1小時學習，比一天花10小時學習來得有效果。而且對於像這種開發技術的知識學習，實作會遠比紙上或網上學習來得重要，看看這些教學就能學會是不可能的，只有親自動手下去作，才有可能真正的學會。
...
React Element(元素)需要透過`ReactDOM.render`方法，才能把虛擬DOM元素，轉換為實體(真實)DOM，也就是說React Element(元素)代表的是一種無狀態的、不可改變的、虛擬的DOM元素，它就是所謂的"**Virtual DOM**"的組成分子。下圖大概簡單說明元素與元件間(你之中有我，我之中有你)，以及大略的執行的關係圖。

![React中元素與元件關係](https://raw.githubusercontent.com/eyesofkids/react-entry-level-book/master/assets/element_component_flow.png)
