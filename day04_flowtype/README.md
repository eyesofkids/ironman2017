...
> "奇異博士"說過「使用警語應該要加註在書的內容的前面」。所以我把注意事先加在這裡。

- 由於Flow還是個年輕的專案，問題仍然很多，功能也沒你想像中完整。對Windows平台的支持也是幾個月前(2016.8)時的事，可能使用時會有一些問題是正常的，但它會愈來愈好。
- 如果你是要學或用React或vue.js等等，Flow是必學的，不管你要用不用，原始碼裡面都用了，不學你可能會看不太懂在寫什麼。
...
> 註: 強(靜態)類型語言，意思是可以讓變數或常數在宣告(定義)時，就限制好只能使用哪種類型，之後在使用時如果發生類型不相符時，就會發出錯誤警告，但不只如些，語言本身也會擴充了更多的類型與語法。

TypeScript自然有它的市場，但它有一些明顯的問題，首先是JavaScript開發者需要進一步學習，內容不少，也有一定陡峭的學習曲線，不過這還算小事。大條的事情是需要把已經正在使用的應用程式碼，都要整個改用TypeScript程式碼語法，才能發揮完整的功效。這對很多已經有內部程式碼庫的大型應用團隊而言，會是個重大的決定，因為如果不往全面更新的路走，將無法發揮強(靜態)類型語言的最大效用。所以許多現行的開放原始碼函式庫或框架，並不會直接使用TypeScript作為程式碼的語言會理由，另一方面當然是TypeScript並非必備的的程式語言技能，而在超集語言界中也有其他的類似語言，也有類似的強類型的設計。當然，TypeScript也有它的優勢之處，自從TypeScript提出了DefinitelyTyped的解決方式之後，讓現有的函式庫能額外再定義出裡面使用的類型，這也是另一個可以與現有框架與庫相整合的方案，這讓許多函式庫與框架都紛紛提交定義檔案。另一個優勢是，TypeScript也是個活躍的開放原始碼專案，發展到現在也有一段時間，算是逐漸成熟的專案。它的背後有微軟(MS)公司的支持，在最近發佈的知名的、全新打造過的Angular2框架中(由Google主導)，也採用了TypeScript作為基礎的開發語言。

現在，Flow提供了另一個新的選項，它是一種強(靜態)類型的輔助檢查工具。Flow的功能是讓現有的JavaScript語法可以先作類型的宣告(定義)，在開發過程中進行自動檢查，當然在最後編譯時，一樣可以用babel工具來移除這些標記。

相較於TypeScript是另外重新制定一套語言，最後再經過編譯為JavaScript程式碼來執行。Flow走的則是非強制與非侵入性的路線。Flow的優點是易學易用，它的學習曲線沒有TypeScript來得高，雖然內容也很多，但大概一天之內學個大概，就可以漸進式地開始使用。而且因為Flow從頭到尾只是個檢查工具，並不是新的程式語言或超集語言，所以它可以與各種現有的JavaScript程式碼相容，如果你哪天不想用了，就去除掉標記就是回到原來的程式碼，沒什麼太大的負擔。當然，Flow的功用可能無法像TypeScript這麼全面性，也不可能改變要作某些事情的語法結構。

總結來說，這兩種方式的目的是有些相似的，各自有優點也有不足之處，青菜蘿蔔各有所好，要選擇哪一種方式就看你的選擇。

不過就上面說的，如果你是要學習React或Vue.js，學習Flow工具是必要的，它們在開發時都採用了Flow工具的Flow類型(Type)的定義方式，相信不久之後週邊的函式庫與元件都會跟著一起使用。
...
利用Flow類型的定義方式，來解決這個小的案例，可以改寫為像下面的程式碼:
...
這訊息是和你說，你這函式的傳入參數是string(字串)類型，與你定義的number(數字)不相符。

複合類型也是很容易可以加註記的，如果這個函式可以使用布林與數字類型，但回傳值可以是數字或字串，就像下面這樣修改過:
...
由上面這個小例子你可以想見，如果在多人協同開發某個大的JavaScript應用時，這種類型的輸出輸入問題，如果利用Flow工具的檢查，可以避免掉許多不必要的類型問題。

## 一個真實案例

可能你會認為Flow工具只能運用在小程式碼中，但實際上Facebook的開發團隊會創造出Flow工具，有很大的原因是為了React與React Native所需要的。

舉一個我最近正在研究的的函式庫程式碼中[NavigationExperimental](https://github.com/facebook/react-native/blob/9ee815f6b52e0c2417c04e5a05e1e31df26daed2/Libraries/NavigationExperimental/NavigationTypeDefinition.js)，這裡面就預先定義所有的物件裡面的結構，像下面這樣的程式碼:
...
Flow具備有像TypeScript語言中，預先定義物件類型的功用(應該算是TypeScript的interface的部份)。上面的都在在這個元件中預先定義的類型，這些類型可以再套用到不同的程式碼檔案之中。

```js
export type NavigationGestureDirection = 'horizontal' | 'vertical';
```

上面這行類似於列舉(enum)的類型，意思是說要不就是'horizontal'(水平的)，要不然就'vertical'(垂直的)，就這兩種字串值可用。
...
`titleOne`代表的是屬性為可自訂的(可有可無)，`titleTwo`代表的是類型可自訂，也就是值的部份除了定義的類型，也可以是null或undefined，不過你還是要給它一個值，屬性仍然需要有。好吧，這講得太細了，如果有用到再查文件就可以。
...
- 直接從Flow的[發佈頁面](https://github.com/facebook/flow/releases)下載可執行檔案，加到電腦中的PATH(路徑)，讓flow指令可以在命令列視窗存取即可。
...
在你的專案根目錄的用命令列工具輸入，這將會建立一個`.flowconfig`檔案，如果檔案已經存在就不需要再進行初始化，這個設定檔一樣是可以加入自訂的設定值，請參考[Advanced Configuration](https://flowtype.org/docs/advanced-configuration.html)這裡的說明，目前有很多專案裡面都已經內附這個設定檔，例如一些React的專案:
...
當有安裝搭配編輯工具的外掛時，編輯工具會輔助顯示，不過有時候會有點頓頓的要等一下，因為檢查的速度還不是那麼快。或是用下面的命令列指令來進行檢查:

```
flow check
```

要特別注意的是，在Visual Studio Code中因為它內建TypeScript與JavaScript的檢查功能，如果要使用Flow工具來作類型檢查，需要在使用者設定中，加上下面這行設定值以免衝突:
...
在我們之前所教的"開發環境與工具"篇中，其中的樣版文件就已經裝好與設定好這個babel擴充外掛，你不用再多安裝了。

## Flow支援的資料類型

Flow用起來是的確是簡單，但裡面的內容很多，主要原因是有很多種不同的使用情況需要搭配。JavaScript裡面的原始資料類型都有支援，但在函式、物件與一些新的ES6標準中的像類別，就會比較複雜，詳細的情況就請到官網中觀看。這裡只能提供一些簡單的介紹說明。
...
這原因是因為雖然輸入時可以用`mixed`，但Flow會認為函式中不見得最後一定會回傳`string`類型，所以會要求你要在函式中的程式碼，要加入檢查對傳入類型在執行期間的類型檢查程式碼，例如像下面修改過才能過關:
...
> 注意: 上面有兩個特例，`typeof null`與`typeof []`都是回傳'object'。也就是說在JavaScript的標準定義中，`null`與`陣列`都是屬於物件類型。所以，Flow工具中的檢查會與JavaScript中有差異，這一點要注意。
...
Flow在最近的[部落格](https://flowtype.org/blog/2016/10/13/Flow-Typed.html)中說明引入了[flow-typed](https://github.com/flowtype/flow-typed/)這種新的函式庫定義檔("libdefs")，這個Github儲存庫中將統一存放所有來自社群提供的定義檔案。這是一種可以讓現有的函式庫與框架，預先寫出裡面使用的類型定義。如果你的專案裡面有使用Flow工具，就可以直接使用這些定義檔，結合現有的函式庫與框架來使用。這個作法是參考TypeScript的DefinitelyTyped方式。因為這還是很新的消息(2016.10)，目前加入的函式庫還沒有太多，不過React週邊的一些函式庫或元件都已經有開始加入，常用的像underscore、backbone或lodash都已經有加入。
...
本文簡單的說明了Flow工具的功能介紹，以及其中的一些簡要的內容、未來發展之類的。相信看過後，你已經對這個Flow工具有一些認識，其實你已經差不多會用了。Flow是一個很新的工具，但相當的有用，建議每個JavaScript開發者都可以試試，一開使用不用太多學習，這篇文章看完就可以開始用了。複雜的地方就再看看文件即可。

對於每個開發具規模化應用的開發團隊來說，或是開發開放原始碼的函式庫或框架的團隊來說，讓JavaScript具有靜態類型的能力，是一個很重要而且很必要的決定。網路上一直有非常多的超集語言(例如TypeScript)的愛好者，會提出要全面改用TypeScript的聲音，例如Vue.js在很早之前就有[討論](https://github.com/vuejs/vue/issues/478)是不是要全面採用TypeScript的聲音。當然後來Vue.js採用了只有提交TypeScript的[DefinitelyTyped](https://github.com/DefinitelyTyped/DefinitelyTyped/tree/master/vue)檔案，而在2.0中採行了Flow工具的定義。這是一個抉擇，在這篇Vue作者對於: [Vue 2.0 为什么选用 Flow 进行静态代码检查而不是直接使用 TypeScript？](https://www.zhihu.com/question/46397274)的內容中，你可以看到他們團隊為何選擇Flow的理由。最後以作者回答的文中總結下面這句話:

> 全部換 TS(TypeScript) 成本過高，短期內並不現實。 相比之下 Flow 對於已有的 ES2015 代碼的遷入/遷出成本都非常低。萬一哪天不想用 Flow 了，轉一下，就得到符合規範的 ES。

以上，Flow提供了另一個選擇，要用什麼工具就看英明的你如何選擇了。
