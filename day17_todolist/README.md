...
這支程式實際上就展示在這個規則下，如何使用一種"迂迴"的方式，讓子元件可以進行自己屬性的更動(或自我毀滅)。當在子元件中觸發某個事件時，實際上它只是遞出訊息告訴父母元件說: `嘿，老爸，我想要改變一下我自己的某個屬性`或是`嘿，老板，麻煩你fire我吧！`，然後由上層的擁有者元件中的方法，來作這個子元件的props(屬性)的更動，最後當然一定要有的，就是整體顯示的重新渲染。
